...
export function getAverageTickRateInMs() {
    return sum(Memory.tickRateMeter.tickRateHistory) / Memory.tickRateMeter.tickRateHistory.length;

}

function updateTickRateHistory(lastTickRate: number): void {
    Memory.tickRateMeter.tickRateHistory.unshift(lastTickRate);
    Memory.tickRateMeter.tickRateHistory = Memory.tickRateMeter.tickRateHistory.slice(0, TICK_RATE_HISTORY_LENGTH);
}
