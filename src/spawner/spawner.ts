...
import getSumOfResourcesToClean, {
  ResourcesToClean,
} from "../roles/hoover/get-sum-of-resourcer-to-clean";
...
export default function updateSpawner(spawn: StructureSpawn): void {
...
function spawnSettlerCreep(spawn: StructureSpawn): void {
  ...
  spawn.spawnCreep(stripBodyParts(SETTLER_BODY_PARTS), name, {
    memory: {
...
function spawnComboSquadAttacker(spawn: StructureSpawn): void {
  const role = "combo-squad-attacker";
  const name = `${role}-${Game.time}`;
...
function doINeedSettler(room: Room): boolean {
  ...
  const doesSettleExists: boolean = Object.values(Game.creeps).some(
    (creep) => creep.memory.role === "settler"
  );
  const isRoomAleadyMine: boolean =
    !!settleFlag && isRoomMine(Game.rooms[settleFlag.pos.roomName]);
  return (
    !!settleFlag &&
...
function doINeedUpgrader(room: Room): boolean {
  const upgradersCount = room
    .find(FIND_MY_CREEPS)
    .filter((creep) => creep.memory.role === "upgrader").length;
  ...
  if (!room.storage) {
    const upgradeContainer:
      | StructureContainer
      | undefined = getUpgraderContainer(room);
    if (
      upgradeContainer &&
      upgradeContainer.store.energy > upgradeContainer.storeCapacity * 0.9
    ) {
...
function doINeedHarvester(room: Room): boolean {
  return (
    getSourceMemoriesWithLackingHarvesterByOriginRoom(room.name).length > 0
  );
}

function doINeedTransporter(room: Room): boolean {
  const isThereAnyStorage: boolean =
    !!room.storage || !!getRoomEarlyStorageContainer(room);

  return (
    isThereAnyStorage &&
    (getSourceMemoriesWithLackingTransporterByOriginRoom(room.name).length >
      0 ||
      (!doesUpgradeTransporterExist(room) &&
        doesUpgradersContainerExist(room)) ||
      !!(room.terminal && doesTerminalNeedTransporter(room.terminal)))
  );
}

function doINeedBuilder(room: Room): boolean {
  const builders: Creep[] = room
    .find(FIND_MY_CREEPS)
  ...
  const workWorthLeftQuickAssumption: number =
    buildersWorkParts * (buildersTicksToLive / 2);
...
function doINeedRefiller(room: Room): boolean {
  return (
    room
      .find(FIND_MY_CREEPS)
      .filter((creep) => creep.memory.role === "refiller").length < 2
  );
}

function doINeedDefender(room: Room): boolean {
  return room.find(FIND_HOSTILE_CREEPS).filter(isCreepDangerous).length > 0;
}
...
function doINeedHoover(room: Room): boolean {
  ...
  return (
    ...
    resourcesToClean.all * MIN_HOOVERS_CAPACITY_TO_DROPPED_RESOURCES_RATIO >
      allHooversCarryCapacity
  );
}

function doINeedLooter(room: Room): boolean {
  return (
    isLootFlagSet() && room.energyAvailable === room.energyCapacityAvailable
  );
}
...
function doINeedScout(room: Room): boolean {
  ...
  const scoutCount: number = Object.values(Game.creeps).filter(
    (creep) =>
      creep.memory.originRoom === room.name && creep.memory.role === "scout"
  ).length;
...
const MIN_HOOVERS_CAPACITY_TO_DROPPED_RESOURCES_RATIO = 0.75;
const MAX_DROPPED_ENERGY_ALLOWED = 300;
const MAX_DROPPED_MINERALS_ALLOWED = 0;
