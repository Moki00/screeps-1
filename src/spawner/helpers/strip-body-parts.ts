...
export default function stripBodyParts(
  ...
  const options2: StripBodyPartsOptionsWithDefaults = defaultsDeep(
    {},
    options,
    defaultOptions
  );

  const bodyPartsLeftByType: BodyPartsCountByType = countBodyPartTypes(
    bodyParts
  );
  ...
  bodyParts.forEach((bodyPart) => {
    ...
    if (minPartsMet && bodyPartsLeftByType[bodyPart] >= requiredParts) {
      ...
      if (minPartsMet && fatigue > options2.fatigue.max) {
        bodyPartsOutput.shift();
        bodyPartsLeftByType[bodyPart] -= 1;
      }
    }
