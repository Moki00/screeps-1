...
export function doesTerminalNeedTransporter(
...
function doesTerminalTransporterExist(terminal: StructureTerminal): boolean {
  return !!terminal.room.find(FIND_MY_CREEPS).find((creep) => {
    return (
      creep.memory.transportResourcesProgram ===
      TRANSPORT_RESOURCES_PROGRAM.FOR_TERMINAL
    );
  });
}

export function hasStorageAnythingThatIsNotEnergy(
  storage: StructureStorage
): boolean {
  return Object.keys(storage.store).length > 1;
}

export function hasTerminalTooLittleEnergy(
  terminal: StructureTerminal
): boolean {
  return terminal.store.energy < TERMINAL_MIN_ENERGY_AMOUNT;
}

export const TERMINAL_MIN_ENERGY_AMOUNT = 20000;
