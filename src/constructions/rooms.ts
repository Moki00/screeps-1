...
export function getRoomWithMyClosestStorageFromPosition(
  position: RoomPosition
): Room | undefined {
  const allMyStorageRooms: Room[] = Object.values(Game.rooms).filter(
    (room) => room.storage && room.storage.my
  );
  ...
  const closestStorage:
    | ClosestRoomResult
    | undefined = allMyStorageRooms.reduce<ClosestRoomResult | undefined>(
    (closestResult, roomWithStorage) => {
      if (!roomWithStorage.storage) {
        return undefined;
      }
      const storage: StructureStorage = roomWithStorage.storage;
      const path: PathFinderPath = PathFinder.search(position, {
        pos: storage.pos,
        range: 1,
      });
      ...
      const isStorageCloser: boolean =
        ...
        (!path.incomplete && path.path.length < closestResult.path.length);
...
function detectAndCreateFirstSpawnsHelpMemory(): void {
  ...
  roomsRequiringFirstSpawnHelp.forEach((roomWithNoSpawn) => {
    const spawnContructionSite:
      | ConstructionSite
      | undefined = roomWithNoSpawn
      .find(FIND_MY_CONSTRUCTION_SITES)
      .find(
        (constructionSite) => constructionSite.structureType === STRUCTURE_SPAWN
      );
    ...
    const helpingRoom:
      | Room
      | undefined = getRoomWithMyClosestStorageFromPosition(
      spawnContructionSite.pos
    );

    if (!helpingRoom) {
      Logger.warning(
        `There is no room which could help room "${roomWithNoSpawn.name}" ` +
    ...
    Logger.info(
      `Room "${helpingRoom.name}" will help room "${roomWithNoSpawn.name}" with its first spawn.`
    );
...
function cleanFirstSpawnsHelpMemory(): void {
  ...
  roomsWithSpawners.forEach((roomWithSpawn) => {
    Object.values(Game.rooms).forEach((room) => {
      const canRoomBeAccessed = !!Game.rooms[room.name];
      const hasHelpedRoomSpawnPlaced = !!(
      ...
      if (
        (hasHelpedRoomSpawnPlaced || !canRoomBeAccessed) &&
        room.memory.anotherRoomsHelp.firstSpawnPosition
      ) {
        Logger.info(
          `Room "${room.name}" stopped helping room ` +
