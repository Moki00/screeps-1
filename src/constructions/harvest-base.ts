...
export default function updateHarvestBases(originRoom: Room): void {
...
export function getHarvesterSourceMemory(
  sourceId: string,
  originRoomName: string
): SourceMemory | undefined {
  const roomNames: string[] = [
    originRoomName,
    ...getRemoteRoomsNames(originRoomName),
  ];
  return getSourceMemoriesByRoomNames(roomNames).find(
    (sourceMemory) => sourceMemory.sourceId === sourceId
  );
}

export function getSourceMemoriesByRoomNames(
  roomNames: string[]
): SourceMemory[] {
  return roomNames
    .filter(
    ...
    .reduce<SourceMemory[]>((acc, roomName) => {
      const thisRoomSourceIdsWithoutHarvesters: SourceMemory[] = Object.values<
        SourceMemory
...
export function getSourceMemoryBySourceId(
  sourceId: string
): SourceMemory | undefined {
  return getAllSourceMemories().find(
    (sourceMemory) => sourceMemory.sourceId === sourceId
  );
}

export function getSourceMemoriesWithLackingHarvester(
  roomNames: string[]
): SourceMemory[] {
  return getSourceMemoriesByRoomNames(roomNames).filter(
    (sourceMemory) => !sourceMemory.harvesterCreepId
  );
}

export function getSourceMemoriesWithLackingHarvesterByOriginRoom(
  originRoomName: string
): SourceMemory[] {
  const roomsWithSources: string[] = [
    originRoomName,
    ...getRemoteRoomsNames(originRoomName),
  ];
...
export function getSourceMemoriesWithLackingTransporter(
  roomNames: string[]
): SourceMemory[] {
  return getSourceMemoriesByRoomNames(roomNames).filter(
    (sourceMemory) =>
      sourceMemory.containerId && !sourceMemory.transporterCreepId
  );
}

export function getSourceMemoriesWithLackingTransporterByOriginRoom(
  originRoomName: string
): SourceMemory[] {
  const roomsWithSources: string[] = [
    originRoomName,
...
export function getHarvestingPositionBySourceId(
  sourceId: string
): RoomPosition | null {
  ...
  if (!source.room.memory.sources[sourceId]) {
    Logger.warning(
      `No such source id "${sourceId}" in room ${source.room.name}.`
    );
    return null;
  }
  ...
  const { x, y } = source.room.memory.sources[sourceId].harvestingPosition as {
    x: number;
    y: number;
  };
...
export function getHarvestContainerBySourceId(
  sourceId: string
  ...
  const harvestingPosition: RoomPosition | null = getHarvestingPositionBySourceId(
    source.id
  );
  ...
  const container: StructureContainer | undefined = harvestingPosition
    ...
    .find((structure) => structure.structureType === STRUCTURE_CONTAINER) as
    | StructureContainer
    | undefined;
...
function getHarvesterBySourceId(sourceId: string): Creep | undefined {
  return Object.values(Game.creeps).find(
    (creep) => creep.memory.targetSourceId === sourceId
  );
}

function getTransporterBySourceId(sourceId: string): Creep | undefined {
  const sourceMemory: SourceMemory | undefined = getSourceMemoryBySourceId(
    sourceId
  );
  if (!sourceMemory) {
  ...
  return Object.values(Game.creeps).find(
    (creep) => creep.memory.transportFromObjectId === sourceMemory.containerId
  );
}

function checkIfContainerExists(roomName: string): void {
  getSourceMemoriesByRoomNames([roomName]).forEach((sourceMemory) => {
    const harvestContainer: StructureContainer | null = getHarvestContainerBySourceId(
      sourceMemory.sourceId
    );
    Memory.rooms[roomName].sources[
      sourceMemory.sourceId
    ].containerId = harvestContainer ? harvestContainer.id : null;
  });
}

function checkIfHarvestersExist(roomName: string): void {
  getSourceMemoriesByRoomNames([roomName]).forEach((sourceMemory) => {
    const creep: Creep | undefined = getHarvesterBySourceId(
      sourceMemory.sourceId
    );
    Memory.rooms[roomName].sources[
      sourceMemory.sourceId
    ].harvesterCreepId = creep ? creep.id : null;
  });
}

function checkIfTransportersExist(roomName: string): void {
  getSourceMemoriesByRoomNames([roomName]).forEach((sourceMemory) => {
    const creep: Creep | undefined = getTransporterBySourceId(
      sourceMemory.sourceId
    );
    Memory.rooms[roomName].sources[
      sourceMemory.sourceId
    ].transporterCreepId = creep ? creep.id : null;
  });
}
...
export function initRoomSourcesMemory(room: Room, originRoom: Room): void {
  ...
  sources.forEach((source) => {
    const harvestingPosition: RoomPosition | undefined = findHarvestingPosition(
      source,
      originRoom
    );
    if (!harvestingPosition) {
...
function findHarvestingPosition(
  source: Source,
  originRoom: Room
): RoomPosition | undefined {
  const anySpawn: StructureSpawn | undefined = originRoom
    .find(FIND_MY_SPAWNS)
    .find(() => true);
  if (!anySpawn) {
    Logger.error(
      `Can't find harvesting position. Can't find path from ${source} to ${originRoom}.`
    );
    return;
  }

  const pathSteps: PathStep[] = source.room.findPath(source.pos, anySpawn.pos, {
    ignoreCreeps: true,
  ...
  return new RoomPosition(pathSteps[0].x, pathSteps[0].y, source.room.name);
}

function createHarvestingConstructionSites(room: Room, originRoom: Room): void {
  const spawn: StructureSpawn | undefined = originRoom
    .find(FIND_MY_SPAWNS)
    .find(() => true);
  const sources: Source[] = room.find(FIND_SOURCES);
  ...
  sources.forEach((source) => {
    ...
    room.memory.sources[source.id].harvestingPosition = simplyfyRoomPosition(
      harvestingPosition
    );
  });
}
...
function findCloseRemoteRoomsNames(room: Room): string[] {
  return getExitRoomsNames(room.name).filter((roomName) =>
    Object.keys(Memory.rooms).includes(roomName)
  );
}

function findRemoteHarvestableRoomsNames(room: Room): string[] {
  const remoteRooms: string[] = findCloseRemoteRoomsNames(room).filter(
    (roomName) => {
      const hasRoomOnlyOneSource: boolean = getRoomSourcesCount(roomName) === 1;
      return hasRoomOnlyOneSource && !isRoomMine(Game.rooms[roomName]);
    }
  );
