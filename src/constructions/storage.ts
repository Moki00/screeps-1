...
function createStorageConstructionSite(spawn: StructureSpawn): void {
  const isStorageAvailable: boolean =
    ...
    CONTROLLER_STRUCTURES[STRUCTURE_STORAGE][spawn.room.controller.level] > 0;
...
function createEarlyStorageContainerConstructionSite(
...
export function getEarlyStorageContainerPosition(
  spawn: StructureSpawn
): RoomPosition {
  return new RoomPosition(spawn.pos.x, spawn.pos.y - 1, spawn.room.name);
}

export function getRoomEarlyStorageContainer(
  room: Room
