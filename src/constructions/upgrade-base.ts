...
export function getUpgraderContainer(
  room: Room
  ...
  const container: StructureContainer | undefined = upgradingPosition
...
export function getUpgradingSpeed(controller: StructureController): number {
  return (
    controller.progress - controller.room.memory.controller.previousProgress
  );
}
...
export function getTransporterByControllerId(
  controllerId: string
): Creep | undefined {
  const controller: StructureController | null = Game.getObjectById<
    StructureController
  >(controllerId);
  ...
  const upgraderContainer:
    | StructureContainer
    | undefined = getUpgraderContainer(controller.room);
  if (!upgraderContainer) {
    return undefined;
  }

  return controller.room.find(FIND_MY_CREEPS).find((creep) => {
    return (
...
function checkIfTransporterExist(room: Room): void {
  ...
  const creep: Creep | undefined = getTransporterByControllerId(
    room.controller.id
  );
  if (creep) {
...
function createUpgradingSpot(room: Room): void {
  const upgradingPosition: RoomPosition | undefined = findUpgradingPosition(
    room
  );
  ...
  room.memory.controller.upgradingPosition = simplyfyRoomPosition(
    upgradingPosition
  );
}

function findUpgradingPosition(room: Room): RoomPosition | undefined {
  const spawn: StructureSpawn | undefined = room
    .find(FIND_MY_SPAWNS)
    .find(() => true);
  ...
  const pathSteps: PathStep[] = room.findPath(controller.pos, spawn.pos, {
    ignoreCreeps: true,
    ignoreDestructibleStructures: true,
    ignoreRoads: true,
  });

  return new RoomPosition(pathSteps[0].x, pathSteps[0].y, room.name);
}

function createAntiDowngradeRampartsAround(
...
function initControllerMemory(room: Room): void {
  room.memory.controller = {
    ...
    previousProgress:
      room.memory.controller && room.memory.controller.previousProgress
        ? room.memory.controller.previousProgress
        : 0,
