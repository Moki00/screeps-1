...
export default function drawRclStats(room: Room): void {
  ...
  const progressToUpgrade: number =
  ...
  const realTimeToUpgradeInMs: number =
    ticksToUpgrade * getAverageTickRateInMs();

  room.visual.text(
    `next upgrade in ≈ ${ticksToUpgrade}t (≈ ${formatRealTime(
      realTimeToUpgradeInMs / 1000
    )})`,
    room.controller.pos,
    getTextStyle()
  );
