...
export function goToSquadRecruitmentPosition(
  creep: Creep,
  options: GoToSquadLineOptions
): void {
  if (!Memory.squads) {
    return;
  }
  ...
  const squadSeat: SquadMemorySeat | undefined = Memory.squads[
    creep.memory.squadName
  ].seats.find((seat) => seat.assignedCreepId === creep.id);
...
function assignCreepToSquadSeat(creep: Creep, squad: SquadMemory): void {
  const assignedSeatIndex: number = squad.seats.findIndex((seat) => {
...
function isSquadFull(squad: SquadMemory): boolean {
  const anyFreeSeat: SquadMemorySeat | undefined = squad.seats.find(
    (seat) => !seat.assignedCreepId
  );
  return !anyFreeSeat;
}
