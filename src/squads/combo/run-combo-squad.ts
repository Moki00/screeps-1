...
export default function runComboSquad(squad: SquadMemory): void {
    ...
    switch (squad.state) {
        ...
        case ComboSquadState.FIGHT:
            const room: Room | undefined = Game.flags[COMBO_SQUAD_ROOM_TARGET_FLAG_NAME].room;
            if (room) {
...
function getSquadCreeps(squad: SquadMemory): Creep[] {
    return squad.seats
        ...
        .map((seat) => Game.getObjectById(seat.assignedCreepId!))
        .filter((creep) => creep !== null) as Creep[];
}

function pickKillTarget(room: Room): Creep | undefined {
    const enemies: Creep[] = room.find(FIND_HOSTILE_CREEPS)
        .sort((creepA, creepB) => countAttackParts(creepA) - countAttackParts(creepB));
...
function pickHealTarget(squad: SquadMemory): Creep | undefined {
    const creeps: Creep[] = getSquadCreeps(squad)
        .sort((creep) => {
...
function getMostInjuredCreep(creeps: Creep[]): Creep | undefined {
    creeps.sort((creepA, creepB) => getPercentageOfHits(creepA) - getPercentageOfHits(creepB));
    if (!creeps.length) {
        return undefined;
    }
...
export const COMBO_SQUAD_ROOM_TARGET_FLAG_NAME: string = 'combo-squad-attack';
