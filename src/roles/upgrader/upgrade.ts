...
export default function upgrade(creep: Creep): void {
  ...
  const upgradingPosition: RoomPosition | null = getUpgradingPosition(
    creep.room
  );
  ...
  const isCreepOnUpgradingContainer: boolean = creep.pos.isEqualTo(
    upgradingPosition
  );
  ...
  const container: StructureContainer | undefined = getUpgradingContainer(
    creep.room
  );
