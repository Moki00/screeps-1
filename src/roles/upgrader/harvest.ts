...
export default function harvest(creep: Creep): void {
  ...
  const upgradeContainer: StructureContainer | undefined = getUpgraderContainer(
    creep.room
  );

  const hasUpgradeContainerEnoughEnergy: boolean =
    !!upgradeContainer &&
    upgradeContainer.store.energy > upgradeContainer.storeCapacity * 0.1;

  if (
    creep.carry.energy >= creep.carryCapacity ||
    hasUpgradeContainerEnoughEnergy
  ) {
