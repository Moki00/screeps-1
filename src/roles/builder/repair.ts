...
export default function repair(creep: Creep): void {
    ...
    const repairTarget: Structure | null = Game.getObjectById(creep.memory.repairTargetObjectId);
    ...
    if (!repairTarget) {
        Logger.warning(`${creep} repair target doesn't exist. Was it destroyed in the meantime?`);
        creep.memory.repairTargetObjectId = undefined;
...
function findRepairTarget(creep: Creep): Structure | undefined {
    const container: StructureContainer | undefined = findContainerToRepair(creep);
    if (container) {
        return container;
    }

    const rampartToRepair: StructureRampart | undefined = findRampartToRepair(creep);
    if (rampartToRepair) {
...
function findContainerToRepair(creep: Creep): StructureContainer | undefined {
    ...
    if (earlyStorageContainer && (earlyStorageContainer.hits < earlyStorageContainer.hitsMax)) {
...
function findRampartToRepair(creep: Creep): StructureRampart | undefined {
    return creep.room.find<StructureRampart>(FIND_MY_STRUCTURES)
        .filter((myStructure) => {
            return myStructure.structureType === STRUCTURE_RAMPART && doesRampartRequireRepair(myStructure);
        })
