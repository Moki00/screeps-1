...
export default function build(creep: Creep): void {
  if (!creep.memory.buildTargetObjectId) {
    const constructionSite: ConstructionSite | undefined = findBuildTarget(
      creep.room
    );
    creep.memory.buildTargetObjectId = constructionSite
      ? constructionSite.id
      : undefined;
  }
  ...
  const buildTarget: ConstructionSite | null = Game.getObjectById(
    creep.memory.buildTargetObjectId
  );
...
function findBuildTarget(room: Room): ConstructionSite | undefined {
  const anotherRoomFirstSpawnSite:
    | ConstructionSite
    | undefined = getAnotherRoomFirstSpawnConstructionSite(room);
  if (anotherRoomFirstSpawnSite) {
    return anotherRoomFirstSpawnSite;
  }

  const constructionSites: ConstructionSite[] = room
    .find(FIND_CONSTRUCTION_SITES)
    .sort((constructionA, constructionB) => {
      const sortByExtensionFirstResult: number =
        constructionA.structureType === STRUCTURE_EXTENSION ? -1 : 1;

      if (sortByExtensionFirstResult !== 0) {
        return sortByExtensionFirstResult;
      }

      const sortByLargestProgressFirst =
        constructionB.progress - constructionA.progress;
      return sortByLargestProgressFirst;
    });
  return constructionSites.find(() => true);
}

function getAnotherRoomFirstSpawnConstructionSite(
  room: Room
  ...
  return anotherRoomFirstSpawnPosition
