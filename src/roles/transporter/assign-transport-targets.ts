...
export default function assignTransportTargets(creep: Creep): void {
  const sourceMemoryWithLackingTransporter:
    | SourceMemory
    | undefined = getSourceMemoriesWithLackingTransporterByOriginRoom(
    creep.memory.originRoom
  ).find(() => true);
  const storageContainer:
    | StructureStorage
    | StructureContainer
    | undefined = getTargetStorage(creep.room);

  if (sourceMemoryWithLackingTransporter) {
    const harvesterContainer: StructureContainer | null = getHarvestContainerBySourceId(
      sourceMemoryWithLackingTransporter.sourceId
    );
    ...
    if (harvesterContainer && storageContainer) {
      ...
      creep.memory.transportResourcesProgram =
        TRANSPORT_NEEDED_RESOURCES_PROGRAM.ENERGY_ONLY;
      return;
    }
  }
  ...
  const doesUpgraderTransporterExist = !!(
  ...
  if (creep.room.storage && terminal && doesTerminalNeedTransporter(terminal)) {
    ...
    creep.memory.transportResourcesProgram =
      TRANSPORT_NEEDED_RESOURCES_PROGRAM.FOR_TERMINAL;
    return;
  }
}
...
function getTargetStorage(
  room: Room
): StructureStorage | StructureContainer | undefined {
  const earlyStorage:
    | StructureContainer
    | undefined = getRoomEarlyStorageContainer(room);
