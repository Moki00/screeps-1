...
export default function claim(creep: Creep): void {
  ...
  const isCreepCarryingEnoughEnergyToRcl2: boolean =
    creep.carry.energy >= CONTROLLER_LEVELS[1];
  ...
  if (!settleFlag) {
    Logger.warning(
      `There is no "${SETTLER_FLAG_NAME}" flag for "${creep.name}" creep to follow.`
    );
    recycle(creep);
    return;
  }
  ...
  const isCreepInTargetRoom: boolean =
    creep.room.name === settleFlag.pos.roomName;
  ...
  const controllerToClaim: StructureController | undefined =
    roomToClaim.controller;

  if (!controllerToClaim) {
    Logger.error(
      `Room "${roomToClaim.name}" doesn't have controller, therefore can't be claimed.`
    );
    recycle(creep);
    return;
  }
  ...
  const claimReturnCode: ScreepsReturnCode = creep.claimController(
    controllerToClaim
  );
  switch (claimReturnCode) {
    ...
    case ERR_NO_BODYPART:
      Logger.warning(
        `Creep "${creep.name}" doesn't have claim parts to claim "${roomToClaim.name}" controller`
      );
      break;
  }
}
