...
export default function runHarvesterRole(creep: Creep): void {
  if (!creep.memory.targetSourceId) {
    const newSourceMemoryToHarvest:
      | SourceMemory
      | undefined = getSourceMemoriesWithLackingHarvesterByOriginRoom(
  ...
  const targetSourceMemory: SourceMemory | undefined = getHarvesterSourceMemory(
  ...
  const isCreepInTargetRoom: boolean =
    creep.room.name === targetSourceMemory.harvestingPosition.room;

  if (!targetSourceMemory.harvestingPosition.room) {
    return;
  }
...
function repairHarvestersContainer(creep: Creep): void {
  const container: StructureContainer | undefined = creep.pos
  ...
  const creepRepairPower: number =
    creep.getActiveBodyparts(WORK) * REPAIR_POWER;
...
function repairHarvestersRampart(creep: Creep): void {
  const rampart: StructureRampart | undefined = creep.pos
    .lookFor(LOOK_STRUCTURES)
    .find((structure) => structure.structureType === STRUCTURE_RAMPART) as
    | StructureRampart
    | undefined;
  if (!rampart) {
    return;
...
function buildHarvestersContainerConstructionSite(creep: Creep): void {
  const constructionSite: ConstructionSite | undefined = creep.pos
...
function saveDroppedEnergy(creep: Creep) {
  const energyResource: Resource | undefined = creep.pos
    .lookFor(LOOK_RESOURCES)
    .find((resource) => resource.resourceType === RESOURCE_ENERGY) as
    | Resource
    | undefined;
  if (!energyResource) {
    return;
