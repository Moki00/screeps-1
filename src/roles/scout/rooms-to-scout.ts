...
function findRoomsNamesToScout(room: Room): string[] {
  return Object.values(getExitRoomsNames(room.name)).filter((roomName) =>
    doesRoomNeedToBeScouted(roomName)
  ) as string[];
}

function doesRoomNeedToBeScouted(roomName: string): boolean {
  const isRoomVisible = !!Game.rooms[roomName];
  const isRoomDataExpired: boolean = isScoutDataExpired(
    getLastScoutUpdateTick(roomName)
  );
...
const SCOUT_ROOM_EXPIRATION_TIME = 2000;
